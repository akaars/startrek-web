...
function gameTemplate() { return `<main class="game"><header class="topbar"><div class="brand">SUPER <b>STAR</b> TREK <span class="eyebrow">WEB MISSION</span></div><div class="top-actions">${languageSwitchTemplate()}<div class="view-select"><button data-view="console" class="${ui.view === 'console' ? 'active' : ''}">01 ${t('console')}</button><button data-view="deck" class="${ui.view === 'deck' ? 'active' : ''}">02 ${t('deck')}</button></div>${ui.view === 'deck' ? `<button class="tool-btn small intel-toggle ${ui.intelHints ? 'active' : ''}" id="intel-hints" title="${intelHintsHint()}" aria-pressed="${ui.intelHints}">${intelHintsLabel()}</button><button class="tool-btn small" id="last-output" aria-label="${t('captainOutput')}" title="${t('captainOutput')}">▤</button>` : ''}<button class="tool-btn small" id="help" aria-label="${t('help')}">?</button><button class="tool-btn small" id="sound">${ui.music ? '♫ ON' : '♫ OFF'}</button><button class="tool-btn small" id="save">${t('save')}</button></div></header>${ui.view === 'console' ? consoleTemplate() : deckTemplate()}${ui.helpOpen ? helpModal() : ''}${ui.outputOpen ? outputModal() : ''}${ui.crystalConfirmOpen ? crystalsModal() : ''}${game.ended ? endModal() : ''}</main>`; }
