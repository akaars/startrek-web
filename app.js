...
const HELP_RU = {
  ...
  move: ['help move', 'Импульсное перемещение по секторам. Формат направления: move <n|s|e|w|ne|nw|se|sw> [число], например move n 2. Формат координат: move <sector X> <sector Y>, например move 1 1. Для цели в другом квадранте: move <quadrant X> <quadrant Y> <sector X> <sector Y>, например move 2 1 5 5. Корабль проходит прямой маршрут и останавливается перед любым объектом на пути.\n\nВремя: за расстояние d в секторах расходуется 0.05 + d / 0.95 звёздной даты. d считается по прямой; при препятствии учитывается только фактически пройденный путь. Импульсный ход быстро расходует лимит времени. Синоним: impulse.\n\nКороткий вызов: m. Пример: m 2 2.'],
  warp: ['help warp', 'Варп-переход. Формат направления: warp <n|s|e|w|ne|nw|se|sw>. Также можно указать целевой квадрант напрямую: warp <quadrant X> <quadrant Y>, например warp 5 2. Enterprise прибывает в ближайший свободный сектор центра указанного квадранта.\n\nВремя: переход на расстояние d квадрантов расходует 0.14 + (1.5 × d) / warp звёздной даты. d считается по прямой.\n\nВарп-фактор: warp <1…10>, например warp 3. Высокий фактор уменьшает расход времени, но требует больше энергии; низкий — экономнее, но медленнее.\n\nКороткий вызов: w. Примеры: w 5 2 и w 3.'],
  ...
  mine: ['help mine', 'Добыча дилития на поверхности. Требуется высаженная посадочная группа и планета с кристаллами. Время: 0.1 + 0.2 × класс планеты звёздной даты — 0.3 для M, 0.5 для N, 0.7 для O. После операции сырые кристаллы окажутся на Enterprise.'],
...
const HELP_EN = {
  ...
  move: ['help move', 'Impulse movement by sectors. Direction format: move <n|s|e|w|ne|nw|se|sw> [number], for example move n 2. Coordinate format: move <sector X> <sector Y>, for example move 1 1. A target in another quadrant uses move <quadrant X> <quadrant Y> <sector X> <sector Y>, for example move 2 1 5 5. Enterprise follows a straight route and stops before any object on its path.\n\nTime: travelling distance d in sectors costs 0.05 + d / 0.95 stardates. d is measured in a straight line; an obstruction charges only for the distance actually travelled. Impulse movement consumes the time limit quickly. Alias: impulse.\n\nShort invocation: m. Example: m 2 2.'],
  warp: ['help warp', 'Warp movement. Direction format: warp <n|s|e|w|ne|nw|se|sw>. You can also specify a target quadrant directly: warp <quadrant X> <quadrant Y>, for example warp 5 2. Enterprise arrives in the nearest free sector to that quadrant’s centre.\n\nTime: travelling distance d in quadrants costs 0.14 + (1.5 × d) / warp stardates. d is measured in a straight line.\n\nWarp factor: warp <1…10>, for example warp 3. A higher factor reduces time cost but uses more energy; a lower factor is slower and more economical.\n\nShort invocation: w. Examples: w 5 2 and w 3.'],
  ...
  mine: ['help mine', 'Mine dilithium on the surface. It requires a landing party and a planet with crystals. Time: 0.1 + 0.2 × planet class stardates — 0.3 for M, 0.5 for N, and 0.7 for O. Raw crystals return with the landing party.'],
...
function sectorRadarTemplate() {
  const labels = Array.from({ length: 10 }, (_, index) => `<span>${index + 1}</span>`).join('');
  const cells = visibleSector().map(symbol => `<span>${escapeHtml(symbol)}</span>`).join('');
  return `<div class="sector-radar" role="img" aria-label="${t('localSector')}"><div></div><div class="sector-radar-columns">${labels}</div><div class="sector-radar-rows">${labels}</div><div class="sector-radar-cells">${cells}</div></div>`;
}
function galaxyChartTemplate() {
  const labels = Array.from({ length: 8 }, (_, index) => `<span>${index + 1}</span>`).join('');
  const cells = game.map.map(quadrant => `<span>${quadrant.known ? escapeHtml(intelCode(quadrant)) : '····'}</span>`).join('');
  return `<div class="galaxy-chart-wrap"><div class="galaxy-chart-heading">${t('starChart')}</div><div class="galaxy-chart" role="img" aria-label="${t('starChart')}"><div></div><div class="galaxy-chart-columns">${labels}</div><div class="galaxy-chart-rows">${labels}</div><div class="galaxy-chart-cells">${cells}</div></div></div>`;
}
...
function scan() {
  ...
  const width = 4;
  const columns = scanned.filter(quadrant => quadrant.y === scanned[0].y).map(quadrant => `Q${quadrant.x}`.padStart(3).padEnd(width)).join(' ');
  const rows = [];
  for (let y = Math.max(1, game.pos.qy - 1); y <= Math.min(8, game.pos.qy + 1); y += 1) {
...
function consoleTemplate() { return `<section class="console-deck"><section class="console-panel"><div class="console-title">${t('localSector').toUpperCase()} <span>Q${game.pos.qx},${game.pos.qy}</span></div>${sectorRadarTemplate()}</section><section class="console-panel"><div class="console-title">${t('galaxy').toUpperCase()} <span>${t('knownSpace').toUpperCase()}</span></div>${galaxyChartTemplate()}<div class="console-keyline">${t('cellLegend').toUpperCase()}</div></section><section class="console-panel console-command"><div class="console-title">${t('commandConsole').toUpperCase()} <span>${t('lastResponse').toUpperCase()}</span></div><div class="terminal-log ${game.output?.kind || ''}" id="log">${escapeHtml(game.output?.text || '')}</div><form class="commandline" id="command"><span>COMMAND&gt;</span><input autocomplete="off" aria-label="${t('commandConsole')}" autofocus placeholder="help · move n 2 · move 1 1 · warp 5 2" /></form></section><section class="console-panel"><div class="console-title">${t('status').toUpperCase()} <span>ENTERPRISE</span></div><pre class="console-ascii status-ascii">${escapeHtml(consoleStatusText())}</pre></section></section>`; }
